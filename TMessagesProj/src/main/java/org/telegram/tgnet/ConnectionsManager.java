...
public class ConnectionsManager {
    ...
    private PowerManager.WakeLock wakeLock;     //后锁
    ...
    public int sendRequest(final TLObject object, final RequestDelegate onComplete, final QuickAckDelegate onQuickAck, final WriteToSocketDelegate onWriteToSocket, final int flags, final int datacenterId, final int connetionType, final boolean immediate) {
        ...
        Utilities.stageQueue.postRunnable(new Runnable() {
            @Override
            public void run() {
                Logs.s("  发送消息send request " + object.toString() + " with token = " + requestToken);
                try {
                    NativeByteBuffer buffer = new NativeByteBuffer(object.getObjectSize());
                    object.serializeToStream(buffer);
                    object.freeResources();

                     Logs.s("   发送地址address   "+buffer.address);
                    native_sendRequest(buffer.address, new RequestDelegateInternal() {
                        ...
                        public void run(int response, int errorCode, String errorText, int networkType) {
                            ...
                            try {
                                ...
                                Logs.s("   发送消息  java received " + resp + " error = " + error);
                    ...
                    Logs.s("  请求参数：onQuickAck  "+onQuickAck+" ###onWriteToSocket### "+onWriteToSocket+"  ###flags### "+flags+ " ###datacenterId###  "+ datacenterId+ " ###connetionType###  "+connetionType+ " ###immediate### "  + immediate+ "  ###requestToken###  "+requestToken);
                } catch (Exception e) {
                    FileLog.e(e);
                }
            }
    ...
    public static native void native_pauseNetwork();    //本机暂停网络
    public static native void native_setUseIpv6(boolean value); //本机设置使用Ipv6
    public static native void native_updateDcSettings();  //本地更新Dc设置
    public static native void native_setNetworkAvailable(boolean value, int networkType); //本机设置网络可用
    public static native void native_resumeNetwork(boolean partial); //本机恢复网络
    public static native long native_getCurrentTimeMillis(); //本机电流时间
    public static native int native_getCurrentTime(); //本地获取当前时间
    public static native int native_getTimeDifference(); //本机有时差
                            //本机发送请求
    ...
    public static native void native_cancelRequest(int token, boolean notifyServer);//本机取消请求
    public static native void native_cleanUp(); //本机清理
    public static native void native_cancelRequestsForGuid(int guid); //本机取消对Guid的请求
    public static native void native_bindRequestToGuid(int requestToken, int guid); //本机绑定请求到Guid
    public static native void native_applyDatacenterAddress(int datacenterId, String ipAddress, int port);//本机适用于数据中心地址
    public static native int native_getConnectionState();//本地连接状态
    public static native void native_setUserId(int id); //本机设置用户Id
    ...
    public static native void native_setProxySettings(String address, int port, String username, String password);//本机设置代理设置
    public static native void native_setLangCode(String langCode); //本机集朗代码
    public static native void native_setJava(boolean useJavaByteBuffers); //本机设置Java
    public static native void native_setPushConnectionEnabled(boolean value); //本机集推连接启用
    public static native void native_applyDnsConfig(int address); //本机适用于Dns配置
    //生成类Guid
