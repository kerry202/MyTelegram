...
public class StickersAlert extends BottomSheet implements NotificationCenter.NotificationCenterDelegate {
    ...
    private TextView previewSendButton;  //预览发送按钮
    private ImageView previewFavButton;  //喜欢预览按钮
    private View previewSendButtonShadow;  //预览发送按钮的影子
    private BackupImageView stickerImageView; //标签图像视图
    private TextView stickerEmojiTextView;      //贴纸Emoji文本视图
    ...
    private void init(Context context) {
        ...
        stickersOnItemClickListener = new RecyclerListView.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (stickerSetCovereds != null) {
                    Logs.s("  stickersOnItemClickListener   1111  ");
                    ...
                    Logs.s("  stickersOnItemClickListener   2222  ");
                    ...
                    for (int a = 0; a < selectedSticker.attributes.size(); a++) {
                        ...
                        if (attribute instanceof TLRPC.TL_documentAttributeSticker) {
                            if (attribute.alt != null && attribute.alt.length() > 0) {
                                ...
                                Logs.s("   发送图片消息数据3  emojiForSticker 1 "+alt);
                                stickerEmojiTextView.setText(Emoji.replaceEmoji(alt, stickerEmojiTextView.getPaint().getFontMetricsInt(), AndroidUtilities.dp(30), false));
                                set = true;
                            }
                            break;
                        }
                    ...
                    if (!set) {
                        String emojiForSticker = StickersQuery.getEmojiForSticker(selectedSticker.id);

                        Logs.s("   发送图片消息数据3  emojiForSticker 2 "+emojiForSticker);

                        stickerEmojiTextView.setText(Emoji.replaceEmoji(emojiForSticker, stickerEmojiTextView.getPaint().getFontMetricsInt(), AndroidUtilities.dp(30), false));
                    }
                    ...
                    TLRPC.FileLocation location = selectedSticker.thumb.location;

                    Logs.s("   发送图片消息数据2  location  "+location);
        ...
        pickerBottomLayout.cancelButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Logs.s("  pickerBottomLayout  setOnClickListener   ");
                dismiss();
            }
        });
        ...
        previewSendButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String trim = previewSendButton.getText().toString().trim();
                Logs.s(" ####previewSendButton "+trim);
                Logs.s(" ####previewSendButton  selectedSticker  "+selectedSticker);
