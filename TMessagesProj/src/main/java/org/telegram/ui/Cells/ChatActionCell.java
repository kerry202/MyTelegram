...
public class ChatActionCell extends BaseCell {
    ...
    public interface ChatActionCellDelegate {
        void didClickedImage(ChatActionCell cell); //并点击图片
        void didLongPressed(ChatActionCell cell);  //并长按
        void needOpenUserProfile(int uid);  //需要开放的用户配置文件
    ...
    private ImageReceiver imageReceiver; //图像接收器
    private AvatarDrawable avatarDrawable;
    private StaticLayout textLayout;    //文本布局
    ...
    private CharSequence customText; //自定义文本

    private ChatActionCellDelegate delegate;  //聊天行动细胞委托
    ...
    public void setMessageObject(MessageObject messageObject) {
        Logs.s("  设置图片消息：setMessageObject   ");
        ...
        if (currentMessageObject.type == 11) {
            Logs.s("  设置图片消息：setMessageObject   1    ");
            int id = 0;
            ...
            if (currentMessageObject.messageOwner.action instanceof TLRPC.TL_messageActionUserUpdatedPhoto) {
                Logs.s("  设置图片消息：setMessageObject  2222 ");
                imageReceiver.setImage(currentMessageObject.messageOwner.action.newUserPhoto.photo_small, "50_50", avatarDrawable, null, 0);
            } else {
                Logs.s("  设置图片消息：setMessageObject  3333 ");
                TLRPC.PhotoSize photo = FileLoader.getClosestPhotoSizeWithSize(currentMessageObject.photoThumbs, AndroidUtilities.dp(604));
                if (photo != null) {
            ...
            Logs.s("  设置图片消息：setMessageObject  4444 ");
    ...
    private void createLayout(CharSequence text, int width) {
        ...
        Logs.s("  createLayoutcreateLayout    text "+text);
