...
public class DialogsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    private ImageView floatingButton; //浮动按钮
    ...
    public View createView(final Context context) {
        ...
        listView.setOnItemClickListener(new RecyclerListView.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                Logs.s(" 首页 聊天消息list click  "+position);
                ...
                if (adapter == dialogsAdapter) {
                    ...
                    if (object instanceof TLRPC.TL_dialog) {
                        ...
                        Logs.s(" 首页 聊天消息list click 222222222222222    "+position);
                        ...
                        Logs.s(" 首页 聊天消息list click 11111111111    "+position);
                    ...
                    Logs.s(" 首页 聊天消息list click 7777777777    "+position);
                    ...
                    if (obj instanceof TLRPC.User) {
                        ...
                        Logs.s(" 首页 聊天消息list click 8888888888    "+position);
                        if (dialogsSearchAdapter.isGlobalSearch(position)) {
                        ...
                        Logs.s(" 首页 聊天消息list click 99999999999999    "+position);
                        dialog_id = ((long) ((TLRPC.EncryptedChat) obj).id) << 32;
                        if (!onlySelect) {
                            dialogsSearchAdapter.putRecentSearch(dialog_id, (TLRPC.EncryptedChat) obj);
                        }
                    } else if (obj instanceof MessageObject) {
                        Logs.s(" 首页 聊天消息list click 101010101010101    "+position);
                        MessageObject messageObject = (MessageObject) obj;
                        dialog_id = messageObject.getDialogId();
                        message_id = messageObject.getId();
                        dialogsSearchAdapter.addHashtagsFromMessage(dialogsSearchAdapter.getLastSearchString());
                    } else if (obj instanceof String) {
                        Logs.s(" 首页 聊天消息list click 212121212121212    "+position);
                ...
                if (onlySelect) {
                    Logs.s(" 首页 聊天消息list click 32323232323232332    "+position);
                    ...
                    //搜索字符串
                    if (searchString != null) {
                        Logs.s(" 首页 聊天消息list click 252525252525252    "+position);
                        if (MessagesController.checkCanOpenChat(args, DialogsActivity.this)) {
                            NotificationCenter.getInstance().postNotificationName(NotificationCenter.closeChats);
                            presentFragment(new ChatActivity(args));
                        }
                    } else {
                        Logs.s(" 首页 聊天消息list click 24242424242424    "+position);
                        if (MessagesController.checkCanOpenChat(args, DialogsActivity.this)) {
                            Logs.s(" 首页 聊天消息list click 252525252525    "+position);
                            presentFragment(new ChatActivity(args));
                        }
                    }
                }
        ...
        if (!onlySelect && dialogsType == 0) {
            ...
            commentView.setDelegate(new ChatActivityEnterView.ChatActivityEnterViewDelegate() {
                ...
                public void onMessageSend(CharSequence message) {
                    Logs.s("    改变消息数据   dialogsActivity    "+message);
                    if (delegate == null) {
                        return;
                    }
